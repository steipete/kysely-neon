...
import ws from "ws";
import { NeonDialect, NeonHTTPDialect } from "../dist/index.mjs";
import { neon } from "@neondatabase/serverless";

const connectionString = process.env.NEON_DATABASE_URL;

if (!connectionString) {
  test("skips Neon integration tests without NEON_DATABASE_URL", (t) => {
    t.pass();
  });
} else {
  const contexts = [
  ...
  for (const ctx of contexts) {
    test.serial(ctx.name, async (t) => {
      ...
      await db.insertInto("kysely_neon_test").values({ email: "info@example.com" }).execute();

      if (ctx.supportsTransactions) {
        await db.transaction().execute(async (trx) => {
          await trx.insertInto("kysely_neon_test").values({ email: "info@example.com" }).execute();
